/*
 * CPUFreq governor ShowTime
 *
 * ShowTime - This is an adaptive gonover CPU, it is designed to balance performance and battery.
 * The difference from schedutil - the fact is that the frequency coefficient has been reduced, now the frequency is exactly as much as needed, but at the same time, in order to balance, I made the response 0-1ms 
 * ShowTime created by Skinger49 (search in telegram, or in tiktok)
 * Device: S20 Ultra Exynos
 * I am ready to cooperate, many people appreciated my ideas, but I don't know how to program.
 * my mail: romanzvini@gmail.com
 */
